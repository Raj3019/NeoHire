...
import { ChevronLeft, ChevronRight, Radio, Crown } from 'lucide-react';
import Link from 'next/link';
import { useAuthStore } from '@/lib/store';
import { jobsAPI, talentRadarAPI } from '@/lib/api';
    ...
    useEffect(() => {
        const fetchRadarStats = async () => {
            try {
                ...
                const totalMatches = res.alerts?.reduce((acc, a) => acc + (a.matchCount || 0), 0) || 0;
                setStats(prev => ({ ...prev, radarMatches: totalMatches }));
            } catch (err) {
                console.error("Failed to fetch radar stats:", err);
            }
        };
            ...
            <div className="min-h-screen bg-neo-bg dark:bg-zinc-950">
                ...
                <div className="max-w-7xl mx-auto px-4 py-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
                        ...
                        <Link href="/recruiter/talent-radar">
                            <NeoCard className="bg-neo-blue border-4 border-neo-black dark:border-white relative overflow-hidden text-white cursor-pointer hover:scale-[1.02] transition-transform shadow-neo-sm">
                                <div className="flex items-center justify-between mb-1">
                                    <h3 className="font-sans text-base text-white font-bold uppercase tracking-wide opacity-90">Talent Radar</h3>
                                    <div className="flex items-center gap-1 bg-neo-black/30 px-2 py-0.5 rounded-full border border-white/20">
                                        <Crown className="w-3 h-3 text-neo-yellow" />
                                        <span className="text-[9px] font-black uppercase tracking-tighter">Premium</span>
                                    </div>
                                </div>
                                <div className="flex items-center gap-2 mt-2">
                                    <p className="text-6xl font-black text-white">{stats.radarMatches}</p>
                                    <div className="bg-white/20 p-1.5 border border-white/30">
                                        <Radio className="w-6 h-6 animate-pulse" />
                                    </div>
                                </div>
                                <div className="absolute bottom-4 right-4 text-[10px] font-mono font-bold bg-white text-neo-blue px-2 py-1 uppercase tracking-widest border-2 border-neo-blue">Manage Radars &rarr;</div>
                            </NeoCard>
                        </Link>
                    </div>
                                    ...
                                    <div className="flex items-center gap-1">
                                        {[...Array(totalPages)].map((_, i) => (
                                            <button
                                                ...
                                                className={`w-8 h-8 font-mono text-xs font-black border-2 transition-all ${currentPage === i + 1
